...
const payMeta = (s) => {
  switch (s) {
    case 'PAID':
      return { label: 'Paid', emoji: '✅', pill: 'bg-emerald-50 text-emerald-700' };
    case 'SUBMITTED':
      return { label: 'To verify', emoji: '🕓', pill: 'bg-amber-50 text-amber-700' };
    case 'PAY_AT_HOSPITAL':
      return { label: 'At reception', emoji: '🏥', pill: 'bg-sky-50 text-sky-700' };
    default:
      return { label: 'Unpaid', emoji: '•', pill: 'bg-ink-100 text-ink-500' };
  }
};
...
export default function Appointments() {
  ...
  // Hospital verifies a UPI payment (or records a cash payment) as received.
  const verifyPayment = async (id) => {
    try {
      await http.patch(`/deopuri/appointments/${id}/payment?status=PAID`);
  ...
  // Payment collection summary across all of this hospital's appointments.
  const payTotals = useMemo(() => {
    const base = data ?? [];
    let collected = 0, toVerify = 0, pending = 0, paidCount = 0;
    base.forEach((a) => {
      const amt = Number(a.amount) || 0;
      if (a.paymentStatus === 'PAID') { collected += amt; paidCount += 1; }
      else if (a.paymentStatus === 'SUBMITTED') { toVerify += amt; }
      else { pending += amt; } // UNPAID or PAY_AT_HOSPITAL
    });
    return { collected, toVerify, pending, paidCount };
  }, [data]);
  ...
  return (
    <section className="animate-fade-up space-y-5 p-4 sm:p-6">
      ...
      <div className="rounded-2xl border border-ink-100 bg-white p-4 shadow-sm">
        <h3 className="mb-3 flex items-center gap-1.5 font-display text-sm font-bold text-ink-900">
          💰 Payments
          <span className="rounded-full bg-emerald-50 px-2 py-0.5 text-xs font-semibold text-emerald-700">{payTotals.paidCount} paid</span>
        </h3>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          <div className="rounded-xl bg-emerald-50 p-3">
            <p className="text-[11px] font-semibold uppercase tracking-wide text-emerald-600">✅ Collected</p>
            <p className="mt-1 font-display text-xl font-bold text-emerald-700">₹{Number(payTotals.collected).toLocaleString('en-IN')}</p>
          </div>
          <div className="rounded-xl bg-amber-50 p-3">
            <p className="text-[11px] font-semibold uppercase tracking-wide text-amber-600">🕓 To verify</p>
            <p className="mt-1 font-display text-xl font-bold text-amber-700">₹{Number(payTotals.toVerify).toLocaleString('en-IN')}</p>
          </div>
          <div className="rounded-xl bg-ink-50 p-3">
            <p className="text-[11px] font-semibold uppercase tracking-wide text-ink-500">⏳ Pending</p>
            <p className="mt-1 font-display text-xl font-bold text-ink-800">₹{Number(payTotals.pending).toLocaleString('en-IN')}</p>
          </div>
        </div>
      </div>
                ...
                {items.map((a, i) => {
                  ...
                  return (
                    <div
                      ...
                      className="group animate-fade-up rounded-2xl border border-ink-100 bg-white p-3 shadow-card transition-all duration-300 hover:-translate-y-0.5 hover:shadow-card-hover"
                    >
                      ...
                      {/* Payment strip */}
                      {(a.amount != null || a.paymentStatus) && (
                        <div className="mt-2.5 flex flex-wrap items-center gap-2 border-t border-ink-50 pt-2.5 text-xs">
                          {a.amount != null && (
                            <span className="font-bold text-ink-800">₹{Number(a.amount).toLocaleString('en-IN')}</span>
                          )}
                          <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 font-semibold ${payMeta(a.paymentStatus).pill}`}>
                            {payMeta(a.paymentStatus).emoji} {payMeta(a.paymentStatus).label}
                          </span>
                          {a.paymentMethod && <span className="text-ink-400">{a.paymentMethod}</span>}
                          {a.paymentRef && <span className="truncate text-ink-400">Ref: {a.paymentRef}</span>}
                          {a.paymentStatus === 'SUBMITTED' && (
                            <button
                              onClick={() => verifyPayment(a.id)}
                              className="ml-auto rounded-lg bg-emerald-600 px-2.5 py-1 font-bold text-white transition hover:bg-emerald-700"
                            >
                              ✅ Verify payment
                            </button>
                          )}
                          {a.paymentStatus === 'PAY_AT_HOSPITAL' && (
                            <button
                              onClick={() => verifyPayment(a.id)}
                              className="ml-auto rounded-lg border border-emerald-300 px-2.5 py-1 font-semibold text-emerald-700 transition hover:bg-emerald-50"
                            >
                              Mark received
                            </button>
                          )}
