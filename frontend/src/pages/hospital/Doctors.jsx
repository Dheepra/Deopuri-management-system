...
import { getDoctors, updateDoctorFee } from '../../services/hospital.js';
...
export default function Doctors() {
  ...
  const [feeModal, setFeeModal] = useState(null); // { id, name, fee }
  const [savingFee, setSavingFee] = useState(false);

  const saveFee = async () => {
    if (feeModal.fee === '' || Number(feeModal.fee) < 0) {
      toast.error('Enter a valid fee');
      return;
    }
    setSavingFee(true);
    try {
      await updateDoctorFee(feeModal.id, Number(feeModal.fee));
      toast.success('Consultation fee updated');
      setFeeModal(null);
      if (refetch) refetch();
    } catch (e) {
      toast.error(e?.response?.data?.message || 'Could not update fee');
    } finally {
      setSavingFee(false);
    }
  };
  ...
  return (
    <section className="space-y-5">
            ...
            return (
              ...
              >
                ...
                <div className="mt-3 grid grid-cols-2 gap-2">
                  ...
                  <div className="col-span-2 rounded-xl bg-brand-50 px-2.5 py-1.5">
                    <p className="text-[10px] font-semibold text-brand-600">🆔 Registration No.</p>
                    <p className="truncate text-xs font-bold text-brand-700">{d?.registrationNumber || '—'}</p>
                  </div>
                </div>

                {/* Consultation fee — hospital admin sets/edits this */}
                <div className="mt-2 flex items-center justify-between rounded-xl bg-emerald-50 px-2.5 py-1.5">
                  <div className="min-w-0">
                    <p className="text-[10px] font-semibold text-emerald-600">💰 Consultation fee</p>
                    <p className="text-sm font-bold text-emerald-700">
                      {d?.consultationFee != null ? `₹${Number(d.consultationFee).toLocaleString('en-IN')}` : 'Not set'}
                    </p>
                  </div>
                  <button
                    onClick={() => setFeeModal({ id: d.id, name: fullName, fee: d?.consultationFee ?? '' })}
                    className="shrink-0 rounded-lg border border-emerald-200 bg-white px-2.5 py-1 text-[11px] font-semibold text-emerald-700 transition-colors hover:bg-emerald-100"
                  >
                    ✏️ Set fee
                  </button>
                </div>
          ...
          <div className="animate-pop flex max-h-[92vh] w-full max-w-lg flex-col overflow-hidden rounded-t-3xl bg-white shadow-2xl sm:rounded-3xl">
            ...
            <form onSubmit={handleDoctorSubmit} className="flex min-h-0 flex-1 flex-col">
              ...
              <div className="flex-1 space-y-4 overflow-y-auto px-5 py-4">
                ...
                <label className="block">
                  <span className="mb-1.5 block text-sm font-semibold text-ink-700">💰 Consultation fee (₹)</span>
                  <input
                    type="number"
                    min="0"
                    className="w-full rounded-xl border border-ink-200 px-3 py-2.5 text-sm text-ink-900 outline-none transition-shadow focus:border-brand-500 focus:ring-2 focus:ring-brand-100"
                    ...
                    onChange={(e) => setDoctorField('consultationFee', e.target.value)}
                  />
                  <p className="mt-1 text-xs text-ink-400">Shown to patients when they book with this doctor.</p>
                </label>
      ...
      {feeModal && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-ink-900/50 p-0 backdrop-blur-sm sm:items-center sm:p-4" onClick={() => setFeeModal(null)}>
          <div className="w-full max-w-sm rounded-t-3xl bg-white p-5 shadow-2xl sm:rounded-3xl" onClick={(e) => e.stopPropagation()}>
            <h2 className="mb-1 flex items-center gap-2 font-display text-lg font-bold text-ink-900">💰 Consultation fee</h2>
            <p className="mb-4 text-sm text-ink-500">Dr. {feeModal.name}</p>
            <label className="block text-sm font-semibold text-ink-700">Fee (₹)
              <input
                type="number"
                min="0"
                autoFocus
                className="mt-1.5 w-full rounded-xl border border-ink-200 px-3 py-2.5 text-sm text-ink-900 outline-none transition-shadow focus:border-brand-500 focus:ring-2 focus:ring-brand-100"
                placeholder="e.g. 300"
                value={feeModal.fee}
                onChange={(e) => setFeeModal({ ...feeModal, fee: e.target.value })}
              />
            </label>
            <p className="mt-1 text-xs text-ink-400">Shown to patients when they book with this doctor.</p>
            <div className="mt-5 flex gap-2">
              <button onClick={() => setFeeModal(null)} className="flex-1 rounded-xl border border-ink-200 py-2.5 text-sm font-semibold text-ink-600 hover:bg-ink-50">Cancel</button>
              <button onClick={saveFee} disabled={savingFee} className="flex-1 rounded-xl bg-brand-600 py-2.5 text-sm font-bold text-white hover:bg-brand-700 disabled:opacity-60">{savingFee ? 'Saving…' : 'Save'}</button>
            </div>
          </div>
        </div>
      )}
